...
const matches = {
    ...
    right: [
        ...
        { id: "r16-12", home: "England", away: "DR Congo", flagH: "🇬🇧", flagA: "🇨🇩" },
...
function createMatchElement(match) {
    ...
    const homeAttr = escapeForAttr(match.home);
    const awayAttr = escapeForAttr(match.away);

    return `
        <div class="match-card bg-zinc-900 border ${selected ? 'border-emerald-500' : 'border-zinc-700'} rounded-3xl p-6">
            <div class="flex items-center justify-between">
                <div onclick="selectWinner('${match.id}', '${homeAttr}')"
                     class="flex-1 flex items-center gap-3 cursor-pointer px-3 py-4 rounded-2xl transition-all ${selected === match.home ? 'selected' : 'hover:bg-zinc-800'}">
                    <span class="text-5xl flag">${match.flagH}</span>
                    <span class="font-semibold text-lg leading-tight">${match.home}</span>
                </div>

                <div class="px-4 text-zinc-500 font-bold text-xl shrink-0">VS</div>

                <div onclick="selectWinner('${match.id}', '${awayAttr}')"
                     class="flex-1 flex items-center gap-3 cursor-pointer px-3 py-4 rounded-2xl transition-all ${selected === match.away ? 'selected' : 'hover:bg-zinc-800'}">
                    <span class="text-5xl flag">${match.flagA}</span>
                    <span class="font-semibold text-lg leading-tight">${match.away}</span>
                </div>
            </div>
        </div>
    `;
...
function generateBracketImage() {
    ...
    element.style.cssText = `position: fixed; top: -9999px; left: -9999px; background: linear-gradient(135deg, #18181b, #27272a); color: white; padding: 40px; width: 1000px; font-family: system-ui; border-radius: 20px;`;
